...
def scf_wavefunction_factory(name, ref_wfn, reference, **kwargs):
    ...
    df_needed = core.get_global_option("SCF_TYPE") in ["DF", "MEM_DF", "DISK_DF", "COSX"]
...
def run_dfocc(name, **kwargs):
    ...
    if name in ['mp2', 'omp2', 'mp2.5', 'mp3', 'lccd', "a-ccsd(t)"]:
...
def run_dfocc_gradient(name, **kwargs):
    ...
    # Shove variables into global space
    if name in ['mp2', 'mp2.5', 'mp3', 'lccd', 'ccsd', 'omp2', 'ccsd(t)']:
        for k, v in dfocc_wfn.variables().items():
            core.set_variable(k, v)
