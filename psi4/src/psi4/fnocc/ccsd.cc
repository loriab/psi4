...
CoupledCluster::CoupledCluster(SharedWavefunction ref_wfn, Options &options) : Wavefunction(options) {
...
PsiReturnType CoupledCluster::CCSDIterations() {
    ...
    if (isccsd) {
        ...
        outfile->Printf("  Begin singles and doubles quadratic ci iterations\n\n");
    }
...
void CoupledCluster::AllocateMemory() {
    ...
    if (!options_.get_bool("RUN_MP2")) {
        ...
        if (isccsd) {
            outfile->Printf(
                "        minimum memory requirements for CCSD =     %9.2lf mb\n",
                8. / 1024. / 1024. * (o * o * v * v + 2. * (o * o * v * v + o * v) + 2. * o * v + 2. * v * v + o + v));
        } else {
            outfile->Printf(
                "        minimum memory requirements for QCISD =    %9.2lf mb\n",
                8. / 1024. / 1024. * (o * o * v * v + 2. * (o * o * v * v + o * v) + 2. * o * v + 2. * v * v + o + v));
        }
...
void CoupledCluster::CPU_I1ab(CCTaskParams params) {
    ...
    for (m = 0, id = 0; m < o; m++) {
        for (e = 0; e < v; e++) {
            for (n = 0; n < o; n++) {
                C_DAXPY(v, -0.5, integrals + m * o * v * v + n * v + e, o * v,
                        tempv + m * o * v * v + e * o * v + n * v, 1);
            }
        }
    }
    ...
    if (isccsd) {
        ...
        for (i = 0; i < nv2tiles - 1; i++) {
            psio->read(PSIF_DCC_ABCI2, "E2abci2", (char *)&integrals[0], v2tilesize * v * o * sizeof(double), addr,
                       &addr);
...
void CoupledCluster::CPU_I1pij_I1ia_lessmem(CCTaskParams params) {
    ...
    for (i = 0; i < o; i++) {
        for (a = 0; a < v; a++) {
            for (m = 0; m < o; m++) {
                C_DAXPY(v, -0.5, integrals + i * o * v * v + m * v + a, o * v,
                        tempv + i * v * v * o + a * v * o + m * v, 1);
            }
        }
    }
        ...
        for (i = 0; i < o; i++) {
            for (j = 0; j < o; j++) {
                for (e = 0; e < v; e++) {
                    ...
                    C_DAXPY(o, -2.0, tempt + i * o * o * v + j * v + e, o * v,
                            tempv + i * o * o * v + j * o * v + e * o, 1);
                }
            }
        }
    ...
    for (a = 0, id = 0; a < v; a++) {
        for (b = 0; b < v; b++) {
            for (i = 0; i < o; i++) {
                ...
                C_DAXPY(o, 1.0, tempv + b * o * o * v + i * v * o + a * o, 1, tempt + a * o * o * v + b * o * o + i * o,
                        1);
            }
        }
    }
...
void CoupledCluster::I2ijkl(CCTaskParams params) {
    ...
    for (j = 0; j < o; j++) {
        for (i = 0; i < o; i++) {
            for (b = 0; b < v; b++) {
                C_DCOPY(v, integrals + j * o * v * v + b * o * v + i * v, 1, tempv + j * o * v * v + i * v * v + b * v,
                        1);
            }
        }
    }
...
void CoupledCluster::I2piajk(CCTaskParams params) {
    ...
    if (isccsd) {
        ...
        for (j = 0; j < novtiles - 1; j++) {
            psio->read(PSIF_DCC_ABCI, "E2abci", (char *)&integrals[0], ovtilesize * v * v * sizeof(double), addr,
                       &addr);
            F_DGEMM('n', 'n', o * o, ovtilesize, v * v, 1.0, tempt, o * o, integrals, v * v, 1.0,
                    tempv + j * o * o * ovtilesize, o * o);
        }
        j = novtiles - 1;
        psio->read(PSIF_DCC_ABCI, "E2abci", (char *)&integrals[0], lastovtile * v * v * sizeof(double), addr, &addr);
        F_DGEMM('n', 'n', o * o, lastovtile, v * v, 1.0, tempt, o * o, integrals, v * v, 1.0,
                tempv + j * o * o * ovtilesize, o * o);
        ...
        for (j = 0; j < o; j++) {
            for (a = 0; a < v; a++) {
                for (i = 0; i < o; i++) {
                    C_DAXPY(o, 1.0, tempt + i * o * o * v + a * o + j, o * v, tempv + j * o * o * v + a * o * o + i * o,
                            1);
                }
            }
        }
...
void CoupledCluster::Vabcd1(CCTaskParams params) {
    ...
    for (i = 0; i < o; i++) {
        for (j = i; j < o; j++) {
            for (a = 0; a < v; a++) {
                ...
                tempv[Position(a, a) * o * (o + 1) / 2 + Position(i, j)] = tempt[a * o * o * v + a * o * o + i * o + j];
            }
        }
    }
    ...
    for (a = 0; a < v; a++) {
        for (b = 0; b < v; b++) {
            for (i = 0; i < o; i++) {
                for (j = 0; j < o; j++) {
                    tempv[a * o * o * v + b * o * o + i * o + j] +=
                        .5 * tempt[Position(a, b) * o * (o + 1) / 2 + Position(i, j)];
                }
            }
        }
    }
            ...
            for (i = 0; i < o; i++) {
                for (j = 0; j < o; j++) {
                    if (i > j)
                        ...
                        sg = sg2;
                    tempv[a * o * o * v + b * o * o + i * o + j] +=
                        .5 * sg * tempt[Position(a, b) * o * (o + 1) / 2 + Position(i, j)];
                }
            }
        }
    }
...
void CoupledCluster::K(CCTaskParams params) {
    ...
    if (isccsd) {
        ...
        for (i = 0; i < o; i++) {
            for (b = 0; b < v; b++) {
                for (j = 0; j < o; j++) {
                    C_DAXPY(v, 1.0, tempv + b * o * o + i * o + j, o * o * v, tempt + i * o * v * v + b * o * v + j * v,
                            1);
                }
            }
        }
        ...
        for (i = 0; i < o; i++) {
            for (b = 0; b < v; b++) {
                for (j = 0; j < o; j++) {
                    C_DAXPY(v, 1.0, tempv + i * o * v + b * o + j, o * o * v, tempt + i * o * v * v + b * o * v + j * v,
                            1);
                }
            }
        }
    ...
    for (a = 0, id = 0; a < v; a++) {
        for (b = 0; b < v; b++) {
            for (j = 0; j < o; j++) {
                ...
                C_DAXPY(o, 1.0, integrals + a * v * o + j * v + b, o * v * v, tempt + a * o * o * v + b * o * o + j * o,
                        1);
                C_DAXPY(o, 0.5, integrals + j * o * v * v + a * v * o + b, v, tempt + a * o * o * v + b * o * o + j * o,
                        1);
                C_DAXPY(o, 0.5, integrals + b * v * o + j * v + a, o * v * v, tempt + a * o * o * v + b * o * o + j * o,
                        1);
            }
        }
    }
...
void CoupledCluster::TwoJminusK(CCTaskParams params) {
    ...
    if (isccsd) {
        ...
        for (i = 0; i < o; i++) {
            for (b = 0; b < v; b++) {
                for (j = 0; j < o; j++) {
                    C_DAXPY(v, 1.0, tempt + i * o * v + j * v + b, o * o * v, tempv + i * o * v * v + b * o * v + j * v,
                            1);
                }
            }
        }
    ...
    if (isccsd) {
        for (i = 0, id = 0; i < o; i++) {
            for (b = 0; b < v; b++) {
                for (int m = 0; m < o; m++) {
                    for (int e = 0; e < v; e++) {
                        tempv[id++] = tb[e * o * o * v + b * o * o + m * o + i] -
                                      0.5 * tb[b * o * o * v + e * o * o + m * o + i] - t1[b * o + m] * t1[e * o + i];
                    }
                }
            }
        }
        ...
        for (i = 0, id = 0; i < o; i++) {
            for (b = 0; b < v; b++) {
                for (int m = 0; m < o; m++) {
                    for (int e = 0; e < v; e++) {
                        tempv[id++] =
                            tb[e * o * o * v + b * o * o + m * o + i] - 0.5 * tb[b * o * o * v + e * o * o + m * o + i];
                    }
                }
            }
        }
    ...
    if (isccsd) {
        for (i = 0; i < o; i++) {
            for (a = 0; a < v; a++) {
                for (b = 0; b < v; b++) {
                    C_DAXPY(o, 1.0, tempv + i * o * v * v + a * o * v + b * o, 1, tempt + i * o * v * v + b * o * v + a,
                            v);
                }
            }
        }
    ...
    for (a = 0; a < v; a++) {
        for (b = 0; b < v; b++) {
            for (i = 0; i < o; i++) {
                C_DAXPY(o, 1.0, tempv + i * v * v * o + b * o * v + a * o, 1,
                        integrals + a * v * o * o + b * o * o + i * o, 1);
                C_DAXPY(o, 1.0, tempv + i + a * o * v + b * o, v * v * o, integrals + a * v * o * o + b * o * o + i * o,
                        1);
            }
        }
    }
    ...
    for (a = 0; a < v; a++) {
        for (b = 0; b < v; b++) {
            for (i = 0; i < o; i++) {
                C_DAXPY(o, 1.0, integrals + b * v * o + i * v + a, o * v * v, tempt + a * o * o * v + b * o * o + i * o,
                        1);
                C_DAXPY(o, 1.0, integrals + i * o * v * v + a * v * o + b, v, tempt + a * o * o * v + b * o * o + i * o,
                        1);
            }
        }
    }
            ...
            for (i = 0; i < o; i++) {
                for (j = 0; j < o; j++) {
                    ...
                    energy += (2. * integrals[iajb] - integrals[jaib]) *
                              (tb[ijab] + fac * t1[(a - o) * o + i] * t1[(b - o) * o + j]);
                    ijab++;
                }
            }
        }
